...
data class PrivacyClassification(
    ...
    companion object {
        fun unknown(reason: PrivacyReason) =
            PrivacyClassification(PrivacyLevel.UNKNOWN, setOf(reason), complete = false)
    }
}
...
data class PrivacyContent(
    ...
    companion object {
        fun from(text: String, history: List<Message>): PrivacyContent = PrivacyContent(
            text = text,
...
fun interface RequestPrivacyClassifier {
...
object PrivacyClassifier : RequestPrivacyClassifier {
    ...
    override fun classify(content: PrivacyContent): PrivacyClassification {
        ...
        val level = when {
            reasons.any { it in sensitive } -> PrivacyLevel.SENSITIVE
            reasons.isNotEmpty() -> PrivacyLevel.PRIVATE
            else -> PrivacyLevel.NORMAL
        }
    ...
    fun classifySafely(
        ...
        classifier: RequestPrivacyClassifier = this
    ): PrivacyClassification = try {
        val result = classifier.classify(content)
        if (!result.complete || result.level == PrivacyLevel.UNKNOWN) {
    ...
    fun classify(text: String): PrivacyLevel = classifySafely(PrivacyContent(text)).level

    fun effective(
        declared: PrivacyLevel,
        automatic: PrivacyClassification
    ): PrivacyLevel {
        if (!automatic.complete || automatic.level == PrivacyLevel.UNKNOWN) return PrivacyLevel.UNKNOWN
        if (declared == PrivacyLevel.UNKNOWN) return automatic.level
        return strongestKnown(declared, automatic.level)
    }

    fun strongest(explicit: PrivacyLevel, detected: PrivacyLevel): PrivacyLevel = when {
        detected == PrivacyLevel.UNKNOWN -> PrivacyLevel.UNKNOWN
        explicit == PrivacyLevel.UNKNOWN -> detected
        else -> strongestKnown(explicit, detected)
    }

    private fun strongestKnown(first: PrivacyLevel, second: PrivacyLevel): PrivacyLevel {
        fun rank(level: PrivacyLevel) = when (level) {
            PrivacyLevel.UNKNOWN -> -1
