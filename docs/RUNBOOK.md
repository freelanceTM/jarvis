...
4. После рестарта: in-memory circuit/метрики сбрасываются (ADR-0001) —
...
4. Пока БД недоступна, лицензии/billing/rate-limit недоступны — это
   осознанный fail-closed (ADR-0001). Android-клиент покажет «сервер
...
## 6. Известные ограничения (осознанные, ADR-0001)
