...
## AGENT-010 — принятое архитектурное решение

`AGENT-010`: «Сравни десять вариантов ноутбуков и выбери лучший в пределах
бюджета» требует актуального каталога, цен и модели ранжирования. На устройстве
нет локального product catalog/ranking capability, а фиктивный Agent-план из
нескольких web-search вызовов не гарантирует десять сопоставимых вариантов.
Поэтому intended route — `CLOUD_AI` с `requiresWeb=true`, а не `AGENT`.
Датасет и regression test обновлены; это больше не routing miss.
